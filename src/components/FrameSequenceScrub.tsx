...
export interface FrameSequenceScrubProps {
  /**
   * Ordered frame URLs (first = approach over water, last = penthouse dusk).
   * Sourced from `flythroughConfig.frameUrls()` — placeholder WebPs now, the
   * real extracted frames later (same folder, same names).
   */
  frameUrls: string[]
  /** Total scroll length of the pinned stage, in vh. Default 300. */
  heightVh?: number
  /**
 ...
 * with priority given to the frames nearest the current scrub position, so
 * scrubbing is usable long before the full set has arrived. Redraw fires
 * only when the resolved frame index actually changes.
 */
export default function FrameSequenceScrub({
  ...
  const staticIndex = Math.min(
    frameCount - 1,
    Math.max(0, reducedMotionFrame ?? 0),
  )
  ...
  const drawFrame = useCallback(
    (index: number, force = false) => {
      ...
      // Nearest loaded frame (search outward) so early scrubbing shows the
      // closest available still instead of black.
      let best = -1
  ...
  // Poster frame first, then sparse keyframes so any scrub position resolves
  // quickly; the in-between fill is deferred until the section approaches.
  useEffect(() => {
  ...
  useLayoutEffect(() => {
    if (reducedMotion || frameCount === 0) {
      ...
      onProgressRef.current?.(0, staticIndex)
      return
    }
